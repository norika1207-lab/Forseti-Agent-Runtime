...
## 19. 段落層級的 coverage 做好了，它抓得到當天那個錯

§17.3 記過：hash 那一層驗不到「讀了幾行」，而「只讀了最後那張表」
正是當天早上實際發生的事，所以那支檢查器抓不到當天那個錯。

現在抓得到。

### 19.1 做了什麼

`apps/forseti-cli/coverage.py`。`ReadRecord` 記一份檔案被讀取涵蓋的
行區間，算出 F08 §5 的七級之一，並說得出沒讀到哪幾行。

`CoverageLog` 是 append-only 的儲存，落在
`.forseti/reading_coverage.jsonl`。**落地而不是放在 context 裡，
理由就是這整份記錄的主題**：一份只存在於 context 的涵蓋記錄，
下一次壓縮就消失，然後新的 session 又從零開始高估自己。

`tools/reading-conformance.py` 的 `coverage_gap()` 從佔位換成真的查詢。

### 19.2 拿當天那個真實情境測

```
只讀最後那張表   SAMPLED 36%   沒讀到 [(1, 225)]   conformant=False
整份讀完         FULL_READ 100%  缺口 []
```

`REQUIRED_READING.md` 有 353 行，第 176 到 223 行是壓縮前寫下的
整組模組規格記錄。`(1, 225)` 這個區間把它完整涵蓋在內。

**「第 176 到 223 行你沒讀進來」——這句話是這個模組存在的理由。**
hash 那一層只說得出「檔案沒變」。

### 19.3 累積與版本的處理

兩次半份的讀取合起來算讀完（先讀前半再讀後半，人確實讀完了），
區間相鄰也併，不然逐段讀完的檔案會因為破碎而看起來有缺口。

但檔案改過之後的記錄不能跟改之前的加在一起，
不然一份被大改過的檔案會因為前後各半份的讀取而看起來讀完了。
`merged_for()` 先按 content_hash 分組再併，有測試守著。

### 19.4 沒有記錄時回 None，不回 0%

**沒有涵蓋記錄不等於沒讀，也不等於讀了。** 回 0% 會冤枉一個真的
讀過但沒記錄的人。跟 `claims.py` 的三態同一個原則。

### 19.5 過程中撞到的兩個 bug，形狀跟本文件其他節一樣

**一，例外被吞成 None。** `coverage_gap()` 原本是
`except Exception: return None`，於是一個壞掉的查詢跟一個誠實的
「沒有記錄」長得一模一樣。實際發生了：動態載入 `coverage.py` 時
`dataclasses` 需要模組先註冊進 `sys.modules`，沒註冊就丟
`AttributeError`，而那個例外被吞掉，畫面顯示「coverage 無記錄」。

修法是兩件：載入順序改對，以及把例外回成
`{"error": ...}` 讓它顯示出來，不再假裝是「沒有記錄」。

**二，這又是「拿容易量的當難量的」。** 見 §18.3。
一個吞掉例外的查詢，是拿「沒有回傳值」當成「沒有資料」。
...
```
ARCH-EXEC-001 到 F08-CTX-001   九份全部 coverage FULL_READ 100%
.forseti/REQUIRED_READING.md   SAMPLED 18%，沒讀到 [(1,175), (241,353)]
```

最後那一行是誠實記的。今天我只讀了那個檔案的第 176 到 240 行，
其餘沒讀。**那一份正是當天事故的那一份，而它現在自己把缺口講出來了。**
