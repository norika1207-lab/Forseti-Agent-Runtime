...
### 階段 3 全量校準：最大的收穫不是誤判（2026-09-11）

同一個方法換一個對象。`tools/owner-audit.py`，30 個 session、
4,139 則真的是她講的話（`NOT_OWNER` 過濾掉系統注入的那些）。
...
CORRECTION 是這一層的定罪輸出（claims 那邊是 REFUTED）。
92 則，2.2%，命中詞分布健康：34 次「你沒有」是最大宗，
長尾都是 1 到 3 次，**沒有哪一條規則在暴衝**。

分類器本來就校準過三輪，所以全量跑的改善幅度小
（CORRECTION 94 到 92，ACKNOWLEDGEMENT 58 到 69）。
這一輪真正的價值在別的地方。

**一，一個假設被推翻了。** 我原本認為 CORRECTION 的主要誤判來源是
「她貼上來的工具輸出」——`owner.py` 的模組註解把它列為已知誤判第一項。
實測：94 則裡命中詞落在程式碼區塊內的有 **0 則**。
那個假設錯了，所以我沒有去實作那個修法。
（跟 B-12 的 exFAT 推論同一個形狀：先驗推論，倒了才找對方向。）

**二，MOVED_ON 的 85% 底下是分類器的 UNKNOWN。**

這是這一輪最重要的發現，而且它不是誤判：

```
MOVED_ON  2,639 輪
  ├─ owner_kind = UNKNOWN       2,242　85%
  └─ owner_kind = NEW_REQUEST     397　15%
```

也就是說，這張沉默地圖現在主要在量的是**「分類器分不出她在說什麼」**，
不是「她跳過了那一段」。

方向是保守的（UNKNOWN 永遠不會變成 EXPLICIT_OK，所以它不會製造
假的同意，`SILENCE` 那段註解擔心的事沒有發生）。但它會讓 `risky`
清單充滿噪音，而 risky 正是階段 3 要給人看的東西。

**所以修法不是把 UNKNOWN 壓下去，是讓統計自己把這件事講出來。**
`silence_summary()` 現在多兩個欄位（`moved_on_from_unknown`、
`moved_on_unknown_share`）與一句 caveat。一個不講自己不確定度的指標，
比沒有指標更危險。

**三，三條有出處的規則修正。** 每一條都附全量跑抓到的實際句子：

- 「ＯＫ」是全形，半形的「OK」在清單裡好好的。加 NFKC 正規化 ——
  那是正規化不是語意判斷，而且存進去的仍然是她真正打出來的字
- 「邏輯要清楚不要搞錯」是預防性指示。判準是位置（錯字詞緊接在
  否定祈使後面），不是語氣
- 「再做一次同步吧」是新要求不是糾正。拿掉「再做一次」，
  理由跟先前拿掉「先做」一模一樣：單獨一句分不出「重來」還是
  「再執行一次」，要分辨得知道上一輪，而這個分類器是無狀態的

**沒修的：** 「因為弄錯了會出人命」仍然判 CORRECTION。
那是假設語氣，要分辨得讀語意，`docs/build-plan.md:350` 禁止。
UNKNOWN 裡那些單字祈使（「停」「掃吧」「全推」「進度」）也沒修 ——
往 `_IMPERATIVE` 加動詞是無止境的，會長成一張沒人知道邊界的表。
它們落到 UNKNOWN 不會製造假證據，只是讓系統少知道一些事。

### 風險清單第一次跑出來：接線暴露的東西比清單本身重要（2026-09-11）

`tools/risky.py` 把 claims、overclaim、owner 三個模組接起來，
產出這套系統真正要給人看的東西：

    在哪幾個地方，AI 說了沒把握的話，而她沒有回應。

交集才是那個縫。單看 flagged 會得到一長串「說過沒把握的話」，
但她可能已經處理了；單看 MOVED_ON 會得到 78% 的輪次，
因為人本來就不會每一段都回應。

**接線過程撞到四個東西，每一個都是介面或判斷的真問題。**

**一，`risky` 本來永遠會是 0。** `silence_map` 以輪為單位，
代表一輪的是最後一塊的行號；而呼叫端標記的是個別文字塊。
兩邊幾乎永遠對不上。**一個永遠回 0 的風險清單，比沒有清單更糟 ——
它看起來像「沒事」。** 修法是輪要記住自己涵蓋哪些行。

**二，理由攤平，清單等於廢了。** 第一次跑出來每一筆的理由長得
一模一樣，因為 `Silence` 沒有把該輪的行號範圍暴露出來，
呼叫端只能把整個 session 的理由全部倒進去。加 `lines` 欄位。

**三，我在「設上限」這件事上連錯兩次，兩次同一個形狀：
以為設了，其實沒有。**

- 第一次：`for f in root.rglob(subject)` 外面包計數器。
  但 rglob 只吐出匹配的項目 —— 一個不存在的檔名在家目錄底下會走完
  整棵樹然後吐出零個，計數器永遠是 0。**限制了產出，沒有限制工作量。**
- 第二次：加快取。擋得住重複的 key，擋不住第一次那幾千個不同的 key。

第三次才對：自己走目錄樹，數走過的目錄。順帶講一件事 ——
一個驗證器為了確認某人有沒有說謊，去翻遍那個人的整個家目錄，
那個代價跟它要回答的問題完全不成比例。

**四，最重要的一個：我根本不知道相對路徑的基準在哪裡。**

兩個基準都試過，兩種誤判方向剛好相反：

```
拿 Forseti repo 當基準   →  dist/index.js、logs/train_run3.log 全被判假
拿 session 的 cwd 當基準 →  docs/build-plan.md、ledger.py 全被判假
                            （這些是這個 repo 裡真的存在的檔案）
```

方向相反，說明問題不在規則。**一個相對路徑少了它的基準，
本來就沒有真假可言。** 而 cwd 也不是基準 —— 這些 session 的 cwd
是家目錄，開發的專案卻在外接碟上。

那個基準只有一個地方有：帳本裡 hook 當時記下的 FILE_WRITE。
所以 `verify()` 現在對「相對路徑而且帳本沒證據」一律回 UNKNOWN。

> **⚠ 同日稍晚更正：上面那句「這些 session 的 cwd 是家目錄」是
> 取樣造成的，不是事實。**
>
> 我抓語料的目錄寫死 `-Users-norikaoda`，而那個目錄名就是 cwd 的編碼 ——
> 它底下只會有 cwd = `/Users/norikaoda` 的 session。從那裡抓六個，
> 然後宣布「沒有 session 的 cwd 是 Forseti repo」，**是同義反覆不是發現**。
>
> `-Volumes-NewDrive-AI-Project-Forseti` 一直都在，底下 21 個 session。
> 是 owner 憑記憶抓到這個錯的，不是我自己發現的。
>
> 「相對路徑需要基準」這個判斷方向仍然成立，`verify()` 的行為保留 ——
> 錯的是我拿來支撐它的語料，不是結論本身。詳見下面「語料目錄改對之後」。

### 跑出來的結果，以及它誠實的意思

```
790 輪　　REFUTED 6　VERIFIED 54　UNKNOWN 1,626
被標記的輪次　2 / 790　（0.3%）
風險清單　1 筆
```

那 1 筆講的是 zeus 上的檔案，本機驗不到 —— 系統說「這裡我沒把握」
其實是對的。

`overclaim` 三個 primitive 都接上了，POSITIVE 是 0。
**接著但沒有貢獻，這件事本身要講出來**，不然下一個人會以為它在工作。
FP-02 與 FP-07 要結構化的 scope 與計數，FP-03 要這個 session 的
receipt 數 —— 全部指回同一個地方。

### 語料目錄改對之後：兩個結果，第二個比第一個重要（2026-09-11 晚）

三支工具的 `DEFAULT_DIR` 從 `-Users-norikaoda` 改成
`-Volumes-NewDrive-AI-Project-Forseti`。順便修掉 `claims-audit.py`
還在硬拿 repo 當所有宣稱基準的問題（同樣的錯我在 `risky.py` 修過，
當時沒回頭改這支）。
...
在 repo 裡講的相對路徑，相對於 repo 解析就找得到。
先前那些誤判全部來自基準錯了，不是規則不好。

**結果二：那 21 個 session 裡沒有人類對話。**

逐個看第一則訊息才發現的：

```
任務 讀 AI-First 工程書第 16-21 節 Phase 7 到 Phase 12
任務 寫 tools/check-orphan-workers.py：檢查帳本裡有沒有被派工但從沒回報的步驟
接手這個專案，告訴我現在該做什麼，還有哪些事沒做完。
在 docs/cases/ 建立一個檔案 hook-probe.md，內容只要一行
```

**19 個是 `forseti.py dispatch` 派出去的 worker，2 個是探針。**

數字直接說明：21 個 session 只有 21 則 owner 訊息 ——
每個剛好一則派工指令，做完就結束。所以沉默地圖是 `UNOBSERVED 100%`，
風險清單是空的。
...
`risky.py` 要看的是「AI 說了沒把握的話，而人沒有回應」。
它需要有來有往的對話。而現在：

```
有對話的地方（cwd = 家目錄）      採集不到      ← B-13
採集得到的地方（cwd = repo）      只有派工，沒有對話
```

工具是好的，資料還不存在。這比原本寫的「機制拿不到資料」具體得多，
而且它指出的下一步很明確：**要有人在 repo 目錄底下跟 AI 真的工作一段。**

2026-09-11 傍晚 owner 選了這條路，已經請 cwd 在家目錄那個 session
移過來重開。

**所以這一輪真正的產出不是那 1 筆清單，是一個結論：
機制都通了，但它們現在拿不到足以判斷的資料。**
B-13 已經從「缺口」改寫成「前提」。
