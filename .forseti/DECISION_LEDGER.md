...
## ADR-010　接受 hook 熱路徑的現況，不為它改架構

**日期** 2026-09-10　**狀態** 已接受（owner 裁決）

**決策：** `docs/build-plan.md:330` 的「hook 端 p95 < 50ms」達不到，
接受現況（p95 220 ms），不為它改成常駐 daemon。

**背景，全部是量出來的：**

| 量的是什麼 | p50 | p95 |
|---|---|---|
| `node` 啟動，什麼都不做 | 90.4 ms | 97.3 ms |
| `node` + import + `appendEvent` | 131.9 ms | 152.3 ms |
| hook 完整端到端 | 142.5 ms | 220.0 ms |

**`node` 啟動本身就是預算的 1.8 倍**，而那一段跟 Forseti 寫了什麼
完全無關。所以這不是「實作不夠快」，是「每次起一個直譯器」這個架構
的下限就在那裡。

**理由：** 220 ms 對一個寫檔操作，使用者感覺不到。而換成常駐 daemon
會帶來一整類新的失效模式 —— 最直接的一個是「daemon 死了誰知道」，
那正是 B-09 花了一整輪在解的問題形狀（worker 死了十七小時沒人發現）。
為了一個使用者感覺不到的延遲，去換一個會製造靜默失效的元件，不划算。

**否決的替代方案：**

改預算，把它改成「相對於 node baseline 的增量 < 50ms」（現在是 +41 ms，
會過）。否決理由是那樣等於為了讓數字好看而重新定義題目 ——
真正該記的是「這個架構的下限是 90 ms」，而不是一個算法上會過的比例。

改架構走常駐 daemon。否決理由見上。

**代價：** `build-plan` 那條預算現在是一個明知達不到的數字。
不刪掉它，改成在 PHASE_STATUS 記下實測與這條 ADR 的指向 ——
一個達不到的目標留著並標明為什麼達不到，比悄悄改小有用。

**推翻條件：** hook 被掛到高頻事件上的時候。現在它只在
`Write|Edit|MultiEdit|NotebookEdit` 與 `Stop` 觸發，一次工作幾十次；
如果哪天接上每個 token 或每次 Read，220 ms × 頻率就會變成真的痛。
另一個訊號是 owner 開始感覺到編輯器有延遲 —— 那時候不必再量，
直接推翻。
