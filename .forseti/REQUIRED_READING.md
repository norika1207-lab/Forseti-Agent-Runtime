...
工程書那一列的字數 2026-09-09 更正過。原本寫 157,680，而那個數字對不上
任何實際檔案（docx 是 123,330，轉出的 md 是 166,196）。表上其他三份用
實測比對都精確吻合，唯獨這一份對不上 —— 因為它先前只有 docx、沒有 md，
所以那個數字沒有來源，是估的。現在填的是 md 的實測大小。

**這件事值得記的不是差了多少，是它為什麼沒被發現：** 表上每個數字看起來
都一樣可信，而其中一個是估的。沒有標記哪些數字有出處，估的跟量的就長得
一模一樣。
...
|---|---|---|
| 階段 0 | 三份入口 + 四份控制檔 | 可以動 |
| 階段 1 事件帳本 | 工程書 Phase 1、v5.0 §6 與 §20 | 工程書那半有對照筆記，**v5.0 §6/§20 仍未讀** |
| 階段 2 Claim/Reality | 工程書 Phase 5、v5.0 §7 | 兩半都有了（v5.0 §7 已讀，工程書有對照筆記） |
| 階段 3 人與 AI 雙向 | v5.0 §8、Formal Spec §5-6 | 已達標 |
| 階段 4 健康與退化 | 工程書 Phase 2-4、v5.0 §13-14 | 工程書那半有對照筆記，**v5.0 §13-14 仍未讀** |
