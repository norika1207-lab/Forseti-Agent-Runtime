 ...
 *
 * ── 這張表的實測結果:不能當判定用,一次都不行 ─────────────
 *
 * 2026-09-08 在 40 個 healthy negative session 上量過
 * (docs/calibration/2026-09-08-healthy-negatives.md):
 *
 *   命中 3664 次 / 15,130 則 assistant 訊息 = 每千則 242 次
 *   40 個 session 全部命中,一個不漏
 *
 * 平均每四則訊息命中一次。§24.1 的 warning storm 門檻是
 * 5 個/10 分鐘,這一個訊號自己就能在幾分鐘內灌爆它。
 *
 * 另一份獨立資料(2026-09-08-self-scan.md)抽查了命中內容,
 * 前 10 則全部是假陽性。最清楚的一則原文是
 * 「不會直接把這段回報文字當成『已驗證』的事實」——
 * 語意跟 coverage 宣稱完全相反,而字串比對分不出來。
 *
 * 所以它在這裡的角色只有一個:讓 detectScopeInflation 的
 * `experimental` 旗標亮起來。ESI 的判定值來自 scope 覆蓋率的算術,
 * 完全不經過這張表。要拿它去產生 finding 之前,先讀那兩份校準資料。
 */
