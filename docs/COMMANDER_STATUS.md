...
| `FOR-P1-001` 持久續作契約 | `STALLED_RECEIPT_CORRECTION` | Ledger / recovery contract / focused tests | Worker 已提交 receipt；獨立重跑 focused suite 為 `55 passed`，但 receipt 的 `changed_files` 漏列目前可觀測的 allowed-scope 檔案。R1 第一次修正與 `git status` 證據矛盾；重派 R1 的回合已結束但未改 receipt、未留下 Git artifact，也未保存完整回歸的最終結果。已發出最後一張 receipt-only R2；在收據可追溯前不得升為 verified。 |
