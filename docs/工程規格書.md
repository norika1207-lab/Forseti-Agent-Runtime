...
十七次裡，十四次是工具或測試抓到的，三次是擁有者抓到的。這個比例會讓工具看起來比實際好，因為十七條裡大部分是普通程式錯誤，不是 Forseti 的目標領域。
...
工具抓到的三條，都還沒造成傷害。造成九小時損失的那一條，是擁有者抓的。
...
只裝在這個 repo，不碰其他目錄。設定在 `.claude/settings.json`，官方文件把那一層定義為「只有含這個檔案的資料夾」。理由與完整說明在 `.claude/README.md`。

兩道保險。第一道是設定檔的位置。第二道在程式碼裡：兩個 hook 從自己的檔案位置算出所屬的 repo 根目錄，`cwd` 不在底下就 `exit 0`，連狀態檔都不建。

第二道存在的理由是第一道依賴我對設定作用範圍的理解，而上次出事正是因為那個理解是錯的。這道邊界不看設定、不看環境變數、不可設定關閉，`test/hooks.e2e.test.mjs` 的 AT-HOOK-B1 到 B4 守著，B4 專門檢查有沒有繞過開關。

還沒驗證的是 Claude Code 是否真的載入那個檔案。驗證要在這個目錄底下開一個 session，寫這段的 session 工作目錄不是這裡。已驗證的是 JSON 合法、命令字串照抄可執行、邊界在 repo 外確實不動作。
