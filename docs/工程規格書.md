...
版本 2.0 · 2026-09-08 · v1.0 的內容全部保留在下方，v2 的增補集中在 §11

> **2026-09-08 稍晚：源頭規格換版**
>
> 擁有者交付了 `Forseti Formal Specification v2.0 (Case-Calibrated)`，已放進
> `docs/spec-v2.0.md`，一字未改。它取代 v0.1 成為現行要求。
>
> v0.1 的 38 條 MUST 沒有被廢止，仍在 `test/spec-v0.1.test.mjs` 逐條驗。
> v2.0 的 93 條 FS 規範句在 `test/spec-v2.test.mjs` 逐條驗。
>
> 這份文件從第 1 節到第 10 節寫的是 v1 的實作，全部仍然成立且仍在跑。
> v2 新增的十四個模組與它們的來源，寫在 §11。兩者的關係是疊加，不是取代：
> v1 那 26 個模組回答「這個 session 的執行期健不健康」，v2 那 14 個回答
> 「宣稱、證據、目標、進度之間的關係有沒有斷掉」。
...
| 21 | v2 的 `receipts` 與 `commitments` 放進 runtime 的 state 卻沒放進 `save()`，hook 每次都是新程序，兩個機制在真實環境安靜地完全不存在 | 想把 v2 接進 hook 的時候。400 多條 v2 測試全在同一個 process 裡跑，一條都沒抓到 |
| 22 | 欄位清單寫死在三個地方（`SECTIONS`、`createSnapshot` 解構、`restore` 組裝），修第 21 條時只改了兩個，序列化看起來完全正常而讀回來是 undefined | 一條跨程序的 runtime 測試 |
| 23 | `spec-v2.test.mjs` 第一次跑報三條 VIOLATION，三條都是測試自己寫錯（正則 `tone` 命中 `milesTONEs`、正則 `llm` 命中自己引用規格書的註解、一個踩在門檻上的邊界值） | 用擁有者的規格書逐條驗自己的時候 |
| 24 | 覆蓋用語的字串比對精確率是 0：掃自己的 session 命中 22 則，抽查前 10 則全是假陽性，最清楚的一則原文是「不會直接把這段回報文字當成**已驗證**的事實」 | 拿 Forseti 掃自己，然後人工抽查命中內容 |
| 25 | 同一個訊號在 40 個 healthy negative 上每千則命中 242 次、40/40 session 全中。一個這種密度的訊號當成 warning，一個人幾分鐘就能灌爆 §24.1 的 storm 門檻 | 撈 40 個真實 session 量命中率 |
| 26 | Incident 聚合層在真實資料上完全失效：63 個 detector hit 變成 63 個 root incident，壓縮率 0。`rootIncidentKey` 把 `evidence_refs` 放進 key，而真實資料每筆證據的時間戳都不同 | 同上。CT-034 用人造資料（共用同一組 evidence_refs）測不到 |
| 27 | 挑 healthy negative 的判準有漏洞：40 個樣本裡 18 個是 subagent transcript，主 agent 的 prompt 被記成 user 訊息，所以「有人類參與」的檢查放它們過關 | 看取樣結果的專案分布 |
| 28 | 兩個字串表訊號的假陽性有五種結構，其中一類**方向是反的**：明確區分「我驗過的」與「我沒驗的」的 agent，會比含糊帶過的命中更多次。一則被標成命中的原文是「為了不再騙你，這句話我不講」 | 抽樣看命中的實際內容，不只看命中率 |
| 29 | 為第 28 條寫的初步分類器 `preliminaryCall` 只抓到五類裡的一類，另外三類全部漏掉 | 拿它的輸出跟自己逐則看的結果比對 |
...
| 模組數 | 40（v1 二十六 + v2 十四），零孤立 |
| 測試套件 | 48 支 |
| 斷言數 | 1543（2026-09-08 實跑，指令見 §6.2） |
| 校準資料 | 130 份 transcript，2026-09-07 |
| hook 實測 | §9.2，三個機制 |
| hook 延遲 | §9.2.1，p50 92 到 206 ms，有界 |
| 對照 v0.1 | 33 CONFORMS / 2 NOT_IMPLEMENTED / 3 NOT_CHECKABLE / 0 違規 |
| 對照 v2.0 | 89 CONFORMS / 0 NOT_IMPLEMENTED / 4 NOT_CHECKABLE / 0 違規；§27 46 條全過 |
| 未實測項目 | 正常工作流程中的自然觸發；§26 的 P14 corpus 校準 |

這份文件裡的每一個數字都可以用 §6.2 的指令重跑驗證。跑出來跟寫的不一樣，以跑出來的為準，並且更新這份文件。

---

## 11. v2.0（2026-09-08 換版之後）

### 11.1 十四個新模組

照 `docs/spec-v2.0.md` §26 的強制實作順序做。FS-IMP-001 明講 P3/P4/P5/P6
未完成前不得把 drift classifier 當主判斷器，所以地基先做，classifier 後做。

| 階段 | 模組 | 職責 | 來源案例 |
|---|---|---|---|
| P0 / P2 | `evidence.js` | 四級 evidence class、Evidence Receipt、時間性證據 | 保留期限刪檔 |
| P3 | `claims.js` | ClaimContract、EvidenceScope、Post-Claim Gate、ESI/PC/SCI | CASE-A / CASE-E |
| P4 | `verifier.js` | VerifierContract、UNKNOWN_COVERAGE、validity 上限 | CASE-E（grep vs hex） |
| P5 | `goalanchor.js` | GAC、七個 drift state、GAR、GDA、FSD、TaskCommitment | CASE-D + OWNER-A |
| P6 | `progress.js` | Activity/Task/Goal 三層、PTN、TOUA、FPR、honesty gap | CASE-C（Bragi） |
| P7 | `sef.js` | 四型 Synthetic Evidence Fabrication、Family C 升級 | CASE-F |
| P7 | `liveness.js` | 六個 task class、SLF、Phantom Verification、TOS、RL | CASE-D / CASE-E |
| P8 | `topology.js` | 節點/邊、UNKNOWN_EDGE、Goal 交集趨勢、路徑重建 | CASE-D |
| P9 | `challenge.js` | Reverse Grill 十問、cross-check、window stage 閘門 | §12 |
| P10 | `risk.js` | Composite R + EvidenceCoverage、EWMA/velocity/persistence | §8 / §9 |
| P11 | `incident.js` | 三層分離、root incident 聚合、去重、Warning Storm | owner 的 >25 warning |
| P12 / P13 | `recovery.js` | L0-L5 ladder、Recovery Capsule、healthy context 選擇 | §16 / §17 / §18 |
| §21 | `primitives.js` | FP-01..FP-25 registry 與其餘十一個 detector | 跨案 |
| §7.5 / §13 | `collaboration.js` | CB、HCD（Human-as-QA） | §13 |

零孤立模組，全部接進 `runtime.js`，共 34 個 v2 API。驗證指令見 §6.2。

### 11.2 三個貫穿 v2 的規則，v1 沒有的

**一，Family C 可以繞過溫度計。**
FS-SEV-001：synthetic evidence 的確定性硬矛盾直接開
`CRITICAL_INTEGRITY_INCIDENT`，不等其他訊號共振。理由是它跟其他失效不同層級：
其他形狀是「證據不夠卻講太滿」，這一類是「產生出來的內容被當成外部發生過的事」。
後者一旦成立，所有以 tool 回傳為基礎的判斷都不再可信，包括 Forseti 自己的。

**二，忙碌不是減輕情節的理由。**
FS-DET-PTN-001 與 FS-SEV-002：承諾的任務零進度而同時很忙，忙碌本身就是那個
任務消失的方式。`progress.js` 的 PTN 分數完全不使用 activity 的大小，
只用它判斷「有沒有在忙」，而且回傳帶 `activity_did_not_reduce_score` 旗標。

**三，Forseti 自己也要被量。**
§24 與 FS-ALR-005：`incident.js` 量自己的 warning 速率、去重率與
GovernanceOverheadRatio。使用者如果在分診 Forseti 的警告，那就是 Human-as-QA
而 Forseti 是失職的那一方。規格書最後一句是這條的來源。

### 11.3 對照源頭規格 v2.0

`node test/spec-v2.test.mjs` 逐條驗 93 條 FS 規範句。

| 判定 | 數量 | 意思 |
|---|---|---|
| CONFORMS | 89 | 有可執行的斷言，而且過了 |
| NOT_CHECKABLE | 4 | 對流程或範圍的要求，不是對程式的要求 |
| VIOLATION | 0 | — |

四條 NOT_CHECKABLE 是 FS-SRC-001（總綱）、FS-CORP-002（語料處理流程）、
FS-CAL-001 與 FS-CAL-002（標註流程）。它們的對應物分別在各 detector 的
`exclusions_checked` 欄位與 `docs/cases/` 的證據等級標註。

零違規是現在的狀態，不是一開始的狀態。第一次跑這份測試報了三條 VIOLATION，
三條都是測試自己寫錯：兩條正則太粗（`tone` 命中 `milesTONEs`、`llm` 命中
自己引用規格書的註解），一條用了剛好踩在門檻上的邊界值。三條都修了測試而不是
改實作，並且在原始碼裡留下為什麼原本會誤判。

### 11.3.1 §27 的 46 條 Conformance Test

`node test/acceptance-v2.test.mjs` 逐條驗，46 條全過，無遺漏。

各模組的測試檔裡本來就標了 CT 編號，但那是散在四十幾支檔案裡的，涵蓋率
只能靠 grep 統計 —— 而 grep 統計不出「這條到底有沒有真的驗到」。這一份把
46 條集中，涵蓋率變成一個跑得出來的數字，缺哪一條會直接讓測試失敗。

第一次盤點時涵蓋 43 條，缺 CT-001、CT-014、CT-031。三條都是做得到的：
前後兩條只是沒有標編號的測試，中間那條需要一個新 detector
（`progress.js` 的 `goalRelevantVerificationAvoidance`，對應 CASE-D 那個
「手上有 production eval set 卻只拿去做 observation」的形狀，§23 把它列為
PS 的 detector input，不是獨立的 FP 編號）。

### 11.4 §26 實作順序的完成狀態

| 階段 | 狀態 | Exit gate 驗證位置 |
|---|---|---|
| P0 術語凍結 | 完成 | `test/evidence.test.mjs` |
| P1 事件帳本 | v1 已有 | `capture.js` + `persist.js` |
| P2 Evidence Receipt | 完成 | 0-byte / 刪檔 / 死 process fixture |
| P3 Claim & Scope | 完成 | ESI/PC/SCI fixture 全過，無 LLM |
| P4 Verifier Registry | 完成 | hex/string 不匹配回 UNKNOWN 而非假反駁 |
| P5 Goal/Task Ledger | 完成 | owner-goal-change 與 task starvation fixture |
| P6 Progress Engine | 完成 | Bragi 式 activity 高 / goal 零 fixture |
| P7 Runtime Detector | 完成 | 空 workflow / stale watcher / 假注入 fixture |
| P8 Topology | 完成 | Mercury 框架替換路徑重建得出來 |
| P9 Bounded Semantic | 完成 | 無全 session 依賴，unknown 保持 unknown |
| P10 Trend/Thermometer | 完成 | 單 spike ≠ failure；惡化趨勢看得見 |
| P11 Incident Aggregator | 完成 | 25 hits 收斂成 3 件事 |
| P12 Intervention | 完成 | 健康流程不被彈窗；硬擋預設關閉 |
| P13 Continuity | 完成 | successor 接續已驗證的血脈而非最新的污染敘事 |
| P14 Corpus calibration | **未做** | 見下 |

P14 是唯一沒完成的一階。它已經開始，但還不足以讓任何門檻升級成 STABLE。

第一筆資料在 `docs/calibration/2026-09-08-self-scan.md`：用 v2 的偵測器掃
寫出 v2 的那個 session。擁有者指出「你自己開發，你自己就是最好的實驗
show case」，而她是對的 —— 一般失效樣本缺的正是 ground truth，只有模型
自己的敘述可用；這個 session 的失效在發生當下就進了 git，每一條都有
commit hash。

那份掃描最重要的結果是它報零 finding，而這個 session 已知有三個真實失效
（見 §6.1 第 21 條）。加上一項精確率為 0 的假陽性，以及一個規格書沒有涵蓋
的形狀（人機比 0.02 的低監督風險），三件事都記在那份文件裡。

第二筆在 `docs/calibration/2026-09-08-healthy-negatives.md`：擁有者交代把
healthy negative 從她其他 session 撈出來。從 1967 個 session、2.1 GB 裡，
用四類獨立於偵測器的 runtime fact 篩出 73 個存活，取樣 40 個，
掃了 15,130 則 assistant 訊息。

那批的結果改變了一個訊號的地位，也暴露了聚合層的一個真實缺陷（第 25、26 條）。
兩件事都已經回寫進程式碼並加測試守住。

**現在缺的不再是樣本，是 owner 的判定。** 40 個 session 上的 3877 次命中，
沒有一次經過人確認是誤報還是真報 —— 而那正是把 hit rate 變成 FPR 唯一的路。
另外仍然缺 owner 那份 >25 warning 的原始清單（§24 原文也說目前沒有）。

在那之前，所有標著 `thresholds_uncalibrated: true` 的門檻都不得升級成 STABLE。

第三筆在 `docs/calibration/2026-09-08-string-signals-verdict.md`：不看命中率，
看命中的**內容**。從那 40 個 session 的命中裡抽 20 則，把命中詞前後各 110 字
抓出來逐則看。

結果是這三份裡最重要的一個：兩個字串表訊號的假陽性有五種結構，
而其中一類的**方向是反的** —— 明確區分「我驗過的」與「我沒驗的」的 agent，
會比含糊說「都處理好了」的 agent 命中更多次，因為前者用了更多第一人稱
驗證動詞。一則被標成命中的原文是「為了不再騙你，這句話我不講」，
那是 agent 正在拒絕做出未經驗證的宣稱。

把誠實行為算成風險訊號，接上任何自動介入都會系統性地懲罰它該獎勵的東西。
...
二，上面那五類結構。字串比對抓得到詞，抓不到詞前面的「不」、抓不到引號、
抓不到後面跟著的數字。這不是詞表不夠長的問題。

兩條都已經寫進程式碼的定義處（`claims.js` 的 `SCOPE_EXPANDING_TERMS`、
`rhetoric.js` 的 `CONFIDENCE_MARKERS`），並各有測試守著那段註解，
以及一條把已知假陽性釘成可重跑事實的測試 —— 哪天有人改進了判準，
那條會失敗，那時候該更新的是測試與校準文件，不是默默讓它過。

在 P14 完成之前，所有標著 `thresholds_uncalibrated: true` 的門檻都不得
升級成 STABLE，而那涵蓋 v2 幾乎每一個數字。這件事寫在每個回傳值裡，
不是只寫在文件裡。
