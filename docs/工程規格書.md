...
第 4、7 兩次是同一類：模組全對，接線漏掉，只有端到端的檢查看得見。
...
import('node:fs').then(async (fs) => {
  const { buildImportRecords } = await import('./src/imports.js');
  const { buildGraph, computeCostVector } = await import('./src/cost.js');
  const files = fs.readdirSync('src').filter(f=>f.endsWith('.js')).map(f=>'src/'+f);
  const g = buildGraph(buildImportRecords(Object.fromEntries(files.map(f=>[f,fs.readFileSync(f,'utf8')]))).imports);
  console.log(files.filter(f => computeCostVector(g,f).d1_count===0 && f!=='src/runtime.js'));
});
"
