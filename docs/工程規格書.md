...
| 測試套件 | 48 支 |
| 斷言數 | 1532（2026-09-08 實跑，指令見 §6.2） |
...
有一個例外，它不需要 owner 判定就成立：`coverage_word` 在 100% 的 healthy
session 上命中、每千則 242 次。不論那些命中是誤報還是真報，一個這種密度的
訊號都不能當 warning。這一條已經寫進 `claims.js` 的定義處。
