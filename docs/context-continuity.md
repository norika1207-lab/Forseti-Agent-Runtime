...
實際上這條結論是錯的。正確答案就在 `~/code-matrix-rebuild/app.py:1297` 放著：那裡有一張一模一樣的中英文動作動詞表，但它不單獨用來下判斷，它只是觸發器，真正定生死的是磁碟有沒有變。假陽性被證據那一關擋掉了。
...
`~/code-matrix-rebuild/app.py`，2026-09-08 完整讀過：
...
| 東西 | 在哪 | 怎麼處理 |
|---|---|---|
| 照妖鏡 `check_honesty` | `~/code-matrix-rebuild/app.py:1329` | 接，不重寫。純函式，輸入是文字加 cwd |
| 循環偵測 `record_behavior` | `~/code-matrix-rebuild/app.py:1382` | 接，不重寫 |
| Token 統計 `token_stats` | `~/code-matrix-rebuild/app.py:1215` | 參考做法，佔用計算自己做 |
...
`~/code-matrix-rebuild/app.py` 的三個函式是現成的，位置在第 10 節的表裡。整份讀完再動，不要 grep 幾行就以為懂了。這是 `bible.md` 的 R-01，而且這份文件的第 1.1 節就是違反它的代價。
